...
class GameBloc extends Bloc<GameEvent, GameState> {
  ...
  void startTimer() {
    ...
    _flameDisplayTimer = Timer(
      Duration(seconds: randomFlameSeconds),
      () {
        if (!_hasSpawnedFlame) {
    ...
    _cookieDisplayTimer = Timer(
      Duration(seconds: randomCookieSeconds),
      () {
  ...
  void _onDisplayCookieEvent(
      DisplayCookieEvent event, Emitter<GameState> emit) {
    emit(state.copyWith(isCookieSpawned: true));
  }

  void _onStartSpeedBoostCountdownEvent(
      StartSpeedBoostCountdownEvent event, Emitter<GameState> emit) {
  ...
  void _startSpeedBoostCountdown() {
    _cookieCountdown?.cancel();
    _cookieCountdown = Timer.periodic(
      const Duration(seconds: 1),
      (_) {
  ...
  void _onSpeedBoostTickEvent(
      SpeedBoostTickEvent event, Emitter<GameState> emit) {
